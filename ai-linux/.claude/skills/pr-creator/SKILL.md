...
```
/pr-creator [--base <ブランチ>] [--draft] [--reviewer <ユーザー,...>] [--label <ラベル,...>] [--template <テンプレートパス>] [--format <フォーマット指示>]
```
...
- **未コミット変更がある場合**: 変更内容を一覧表示し、AskUserQuestionで確認:
...
呼び出し時の引数構成:
- 出力先パス: 一時ファイル `/tmp/pr-body-<branch-name>.md`
...
`/pr-description` スキルが提示するPRタイトル候補（Conventional Commits形式・72文字以内）をユーザーに提示し、AskUserQuestionで選定する。
...
以下の情報をまとめて提示し、AskUserQuestionで「作成」「修正」「中止」を確認する。
