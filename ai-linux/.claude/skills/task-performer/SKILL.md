...
- `prompt`: `タスクファイルパス: <指定された todos/NNN-{task}/README.md のパス> --with-log`（必要なら追加の補足を併記）

`--with-log` を必ず付与する理由: task-performer は前回中断時の状況を `logs/NNN-{task}/` の過去作業ログから再開するため。task-reader はデフォルトで過去ログを読み込まず、`--with-log` 指定時のみ読み込む。
