...
2. `README.md`、`todos/README.md`、`specs/**/*.md`、`references/**/*.md` をファイル名検索で列挙して読み込む。ディレクトリ自体は読み込まない
3. 大きなファイル（目安: 500行超）は関連キーワードで内容検索してから部分読込する。ただし指定タスクファイルは常に全文を保持する
4. 上記の構造化サマリと同じ7項目に整理する。情報不足の場合に限り、該当タスクの過去ログを追加で読み込む
...
2. **git管理されている変更ファイルのステージング**
   ...
   git add {変更したファイル}
   ```
   対象は実装で変更したコード・テスト等のみ。タスクドキュメント（`todos/`・`logs/` 配下等、手順4で更新するファイル）は、それがgit管理下にあっても`git add`/`git commit`の対象に含めない（下記手順4参照）。
3. **コミットログファイルの出力**
