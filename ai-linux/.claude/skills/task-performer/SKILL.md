...
**Why**: 重いテストはローカルで実行環境（DB・コンテナ・認証情報・ネットワーク）の準備が必要で、環境差異による偽陽性・偽陰性が起きやすく、実行コストも高い。これらはCIの統一環境で実行する方が再現性・信頼性が高い。一方、軽量ユニットテストは環境非依存で高速なため、ローカルで即座に回す方がフィードバックループが短く有益。だからこそ「軽いものはローカル、重いものはCI」と役割を分ける。

### 重いテストの判定シグナル

以下を手掛かりに分類する。複数のシグナルを総合して判断し、それでも軽量/重いの判別がつかない場合は AskUserQuestion で確認する。
...
過去ログ（`logs/NNN-{task}/`）は容量が大きくコンテキストを圧迫しやすいため、**既定では読み込まず、必要と判断した場合のみ後追いで取得する**。具体的には次の2段階で委譲する。
...
- `prompt`: `タスクファイルパス: <指定された todos/NNN-{task}/README.md のパス>`（必要なら追加の補足を併記。フラグは付けない）
...
この場合、`task-reader` へ再度委譲する:

- `subagent_type`: `task-reader`
- `prompt`: `タスクファイルパス: <同じパス> --only-log`

`--only-log` は標準ファイルを再読込せず**過去ログのみ**を返すフラグで、段階1で取得済みの内容を二重に読まずに済む。段階1のサマリと段階2の過去ログを突き合わせて Phase 2 の判断材料とする。

**Why（2段階に分ける理由）**: 多くのタスク（初回実装など）では標準ファイル群だけで計画立案でき、過去ログは不要。常に過去ログを読むと容量の大きいログでコンテキストを浪費する。そこで既定は「ログなし」とし、不足を実感したときだけ `--only-log` で差分補完することで、コンテキスト消費を最小化しつつ必要時の情報網羅性を確保する。
...
6. 過去の作業ログ（既定では未取得。段階2の `--only-log` 委譲時に補完される）
...
**コードコメントに含めてはいけない情報:**
- タスクドキュメントのファイルパス・URL（例: `docs/tasks/todos/001-setup/README.md`、ローカルやイントラのタスク管理URL等）
- タスクID・TODO番号への言及（例: 「TODO-123対応」「タスク001の要件で追加」「YYYYMMDD-{name} プロジェクト」等）
- セッション内の作業ログファイルパス（例: `logs/NNN-{task}/...`）や、当該リポジトリ外の個人専用ディレクトリ（`.claude/agent-memory/` 等）への参照

**Why**: コードコメントはソースの一部としてリポジトリに永続化され、変更経緯を追う第三者（将来の自分・新規参画者・OSS閲覧者を含む）が読む。タスクドキュメントやタスクIDは対応者のローカル環境にしか存在せず、読み手は参照できない。残しても「どこを見ればよいか」という誤った期待を生むだけで、混乱の原因になる。コメントはコードとリポジトリ内の情報だけで自己完結させる。

**代わりに書くべき内容**: 「なぜ」その実装にしたか（設計判断の理由・トレードオフ・非自明な前提や注意点）。タスク管理側の文脈はコードに残さず、Phase 7の作業結果レビューやタスクファイル更新側に留める。

### Phase 5: テスト実行

「テスト実行方針」に従い、テストを分類してから実行する。

1. **テストコマンドの確認**: Phase 1 の自動検出結果からテストコマンドを確認する。未特定の場合のみ AskUserQuestion で確認
2. **テストの分類**: 今回の変更が対象とするテストを、判定シグナル（「テスト実行方針」参照）に基づき軽量/重いに仕分ける。判別がつかない場合は AskUserQuestion で確認する
3. **軽量テストの実行**: 軽量テストはローカルで自動実行する。失敗した場合: エラー確認 → 修正 → 再実行を繰り返す
4. **重いテストの扱い**: 重いテスト（DB/integration/E2E）はローカル実行を既定でSKIPし、CIに委ねる
   - ローカル実行が望ましいと判断した場合（例: 変更が重いテストの中核ロジックを直接変える、CIでの再現が難しい）も、**実行前に必ず AskUserQuestion で「ローカル実行するか / CIに委ねるか」を確認する**。無断でローカル実行しない
...
| 軽量テスト環境が未設定 | AskUserQuestionで確認後、環境がない場合はPhase 5の軽量テスト実行をスキップ。スキップした旨をPhase 7の報告に記載（重いテストは元よりCI委譲のため影響なし） |
...
- **コードコメントへのタスクドキュメント参照の混入**（タスクドキュメントのパス・URL、タスクID・TODO番号、ローカル作業ログパス等） — これらは対応者のローカル環境にしか存在せず、コメントを読む第三者が参照できないため（詳細・代替案はPhase 4-2「コードコメントに含めてはいけない情報」参照）
