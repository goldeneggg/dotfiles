...
これにより、標準ファイル群（`todos/NNN-{task}/README.md`・`todos/README.md`・`specs/*.md`・`references/*.md` とプロジェクト README）に加え、`progresses/NNN-{task}/` の成果・進捗管理ドキュメントのサマリが返る。`logs/` の過去ログは含まれない。
...
2. `README.md`、`todos/README.md`、`specs/**/*.md`、`references/**/*.md`、`progresses/NNN-{task}/**/*` をファイル名検索で列挙して読み込む。ディレクトリ自体は読み込まない
