...
```text
[タスクファイルパス] [--commit | --commit-push | --commit-push-pr] [--no-agent] [--auto]
```
...
タスク指示に `--auto` が含まれる場合、Phase 3 は計画提示後、Phase 7 は作業結果報告後かつ全受け入れ条件が `充足` なら、承認待ちを省略する。安全性やスコープに影響しない選択は推奨案または最も安全で可逆な既定値を採用し、Phase 7 で報告する。commit・push・PR作成は暗黙に有効化せず、併用されたオプションの範囲で実行する。
...
全 `AC-ID` をTODOの記載順に評価する。`未充足` または `未確認` が1件でもある場合は完了承認を求めず、必要な追加作業または条件変更の相談へ戻る。

通常モードでは、作業結果の承認に加えて**「追加・申し送りタスク（1xx）をファイルとして起票するか」**も確認する。`--auto` 指定時は全 `AC-ID` が `充足` なら作業結果の承認待ちを省略するが、タスク起票はスコープ拡張に当たるため確認する。`未充足` または `未確認` がある場合は追加作業または条件変更を相談する。

### Phase 8: 承認後の処理

承認されたら以下を実施する。`--auto` 指定時は、Phase 7 で全 `AC-ID` が `充足` と判定されたことを承認相当として扱う:
...
4. **進捗正本の更新**（task-starter形式のファイルパスで提供された場合）
   ...
   - 全 `AC-ID` が `充足` で承認済み（`--auto` では承認相当）の場合だけ、状態を `完了` に更新し、最終更新・完了した作業・残作業・ブロッカー・成果を共有契約の必須形式に揃える
   - 個別正本の更新直後に同期スクリプトを実行し、`progresses/README.md` の該当行が `完了` と同じ最終更新を示すことを確認する。個別正本と一覧の更新を同じ完了処理として扱う
...
| 軽量テスト環境が未設定 | 通常モードではユーザーに確認後、環境がない場合はPhase 5をスキップ。`--auto` 指定時は確認せずスキップ。いずれもPhase 7に記録（重いテストは元よりCI委譲のため影響なし） |
