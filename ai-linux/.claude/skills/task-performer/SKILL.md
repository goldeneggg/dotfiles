...
- task-starter形式では `PROGRESS.md` への作業状態・受け入れ条件の検証結果・証跡の反映
...
**委譲方法:** `progresses/NNN-{task}/PROGRESS.md` は常に必要なため、`task-reader` が既定で読み込む。過去ログ（`logs/NNN-{task}/`）は容量が大きくコンテキストを圧迫しやすいため、**既定では読み込まず、必要と判断した場合のみ `--with-log` を付けて取得する**。利用している実行環境が提供する委譲手段（サブエージェント起動・専門エージェント呼び出し等）を通じて、`task-reader` エージェントに以下の内容を渡して実行を依頼する。
...
これにより、標準ファイル群（`todos/NNN-{task}/README.md`・`todos/README.md`・`specs/*.md`・`references/*.md` とプロジェクト README）に加え、`progresses/NNN-{task}/PROGRESS.md` の進捗・成果サマリが返る。`logs/` の過去ログは含まれない。
...
6. タスク進捗・成果（`progresses/NNN-{task}/PROGRESS.md`。常時取得）
...
2. `README.md`、`todos/README.md`、`specs/**/*.md`、`references/**/*.md`、`progresses/NNN-{task}/PROGRESS.md` を読み込む。`progresses/NNN-{task}/` のその他のファイルは内容を読まず、非標準ファイルとして警告する
