...
タスク指示に `--auto` が含まれる場合、Phase 3 は計画提示後、Phase 7 は作業結果報告後かつ全受け入れ条件が `充足` なら、承認待ちを省略する。安全性やスコープに影響しない選択は推奨案または最も安全で可逆な既定値を採用し、Phase 7 で報告する。`--commit` は暗黙に有効化せず、`git push` の禁止も解除しない。
