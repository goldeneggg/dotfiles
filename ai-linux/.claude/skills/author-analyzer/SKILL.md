...
description: |
  ...
  (4) ユーザーが明示的に「/author-analyzer」を実行した時
...
| 引数なし or authorなし | （空） | AskUserQuestionで author を確認 |
...
リポジトリごとに以下を実行する。**各 gh コマンドは可能な限り並列で実行**してよい（独立な取得のため）。
...
`scripts/aggregate.py` に各データを渡し、集計済みJSONを得る:
