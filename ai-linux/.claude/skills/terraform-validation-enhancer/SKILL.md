...
description: |
  ...
  (6) ユーザーが明示的に「terraform-validation-enhancer スキル」の実行を指示された時
...
3. `.tf` ファイルが見つからない場合、推奨案を含む複数の選択肢の提示で対象ディレクトリをユーザーに確認
...
修正後、以下の検証ループ（`terraform init`/`validate`/`plan`と分析スクリプトの再実行）はサブエージェントに委譲し、メインには合否・plan差分の要点・再分析での改善件数だけを戻させる。`terraform plan` はクラウドプロバイダへの実アクセスを伴い出力も長大になりやすいため、委譲によるコンテキスト削減効果が大きい。対象が小規模な修正で出力が短く収まると分かっている場合は、委譲せずメインで直接実行してよい:
