...
**「確認のため実行する」という行為も含めて禁止。** レビューはdiffの静的読解のみで完結させる。
...
- **絶対禁止: テスト実行・Lint実行・ビルド・コード修正は一切行わない** — このスキルはdiffの静的読解によるレビューのみを行う。「動作確認のため」「確認のため」を理由にした実行も禁止
...
- **スキル名・サブエージェント名をレポートに含めない** — 「pr-reviewer スキルにより」「general-purpose エージェントが」のような記述はレビュー実行環境に固有の情報であり第三者には不要。レポートはツールや実行環境を意識させない純粋なレビュー内容のみで構成する
