...
- **フィードバックの文章品質: Low freedom** — 「フィードバックの提供」と「文章の書き方」に厳密に従う。造語や説明のない略語を使わず、各指摘に理由・本来あるべき姿・ユーザーへの影響を必ず含める
...
問題点と改善点を優先度別に整理し、各指摘を以下の構造で日本語で記載する。項目を省略した指摘は出力しない。

- **指摘**: 現在の実装で何が起きるかを具体的に説明する
- **指摘理由**: なぜ問題と判断したかを、発生条件やコード上の根拠とともに説明する
- **本来あるべき姿**: 期待される動作や満たすべき条件を説明する
- **ユーザーへの影響**: 利用者や運用者が、どの操作でどのような不利益を受けるかを説明する。直接的な影響がない場合はその旨と、将来の不具合や保守への間接的な影響を説明する
- **修正案**: 問題を解消する具体的な方法を説明する
- **該当箇所**: diff 内のファイルパスを `path/to/file.ext:42` 形式で示す
- **根拠資料**: 判断に用いた公式文書などがある場合のみ示す

「指摘理由」ではコードの状態を繰り返すだけでなく、その状態が問題を起こす仕組みを説明する。「ユーザーへの影響」は根拠のない推測で補わず、確認できる範囲と未確認の範囲を区別する。
...
- ⚠️ **条件付きで承認可能**: High 3-5件（重要度に応じて判断）
...
- [ ] 各指摘に「指摘理由」「本来あるべき姿」「ユーザーへの影響」があり、問題が起きる仕組みまで説明できているか
- [ ] 独自の造語、説明のない略語、曖昧な表現を、一般的で具体的な言葉へ直したか
