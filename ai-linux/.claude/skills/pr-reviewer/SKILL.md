...
1. **内容を引用してパスは書かない** — 「タスク要件: ユーザー認証 API を実装する」のように要件文を直接記載する
2. **抽象指示語で参照する** — 「タスク要件 #3 によれば〜」「仕様書では〜」のように、ファイルパスではなく論理的な参照名を使う
...
- [ ] 外部不可視ドキュメントを参照する場合、内容引用または抽象指示語に置き換わっているか
