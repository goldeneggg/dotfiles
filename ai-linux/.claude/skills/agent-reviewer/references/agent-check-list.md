...
### 単一責任の原則
- [ ] Agentが **単一の明確なタスク** に特化しているか（複数の無関係な責務を持っていないか）
- [ ] 「このAgentは何をするか」を **1文** で説明できるか

### 要件の理解
- [ ] Agentの目的が **1-2文** で簡潔に説明できるか
- [ ] 目的は「何ができるか」を明確に述べているか（「便利」「効率的」ではなく）
- [ ] Agentを必要とする具体的な利用場面が示され、目的と境界を判断できるか（件数は固定しない）
- [ ] Agentの **境界** が明確か（何を含み、何を含まないか）

### スコープ設計
- [ ] 保存場所が目的に合っているか
  - `.claude/agents/`: プロジェクト固有（VCSにチェックインしてチーム共有）
  - `~/.claude/agents/`: ユーザー固有（全プロジェクトで利用）
- [ ] プロジェクト固有Agentの場合、VCSにチェックインされているか
...
- [ ] **model** フィールドが目的に対して適切か
  - モデルエイリアス: `sonnet`, `opus`, `haiku`
  - フルモデルID: `claude-opus-4-6`, `claude-sonnet-4-6` 等も使用可能
  - `inherit`: 親会話のモデルを継承（**デフォルト**、未指定時はこれ）
- [ ] モデル選択の **理由** が説明できるか（コスト効率・タスク複雑性のバランス）
...
### メモリ設計
- [ ] **memory** フィールドの必要性が検討されているか
  - `user`: 全プロジェクト横断で学習を蓄積（推奨デフォルト）
  - `project`: プロジェクト固有の知識（VCS共有向け）
  - `local`: プロジェクト固有の知識（VCS除外向け）
- [ ] メモリスコープの **選択理由** が説明できるか
- [ ] メモリ使用時、システムプロンプトに **メモリ参照の指示** が含まれているか（「作業開始前にメモリを確認」等）
- [ ] メモリ使用時、システムプロンプトに **メモリ更新の指示** が含まれているか（「作業完了後に新たな学びを保存」等）
- [ ] メモリを使用しない場合、その判断が妥当か（単発タスクで学習蓄積が不要等）

### 隔離設計（Isolation）
- [ ] **isolation** フィールドの必要性が検討されているか
  - `worktree`: 一時的なGitワークツリーで隔離されたリポジトリコピーを使用
  - 変更なしの場合は自動クリーンアップされる
- [ ] コード変更を行うAgentで、メインブランチへの **影響リスク** が考慮されているか
- [ ] 並行実行が想定されるAgentで **コンフリクト回避** が検討されているか
- [ ] 隔離が不要な場合（読み取り専用Agent等）、その判断が妥当か

### 実行制限設計（maxTurns）
- [ ] **maxTurns** フィールドの設定が検討されているか（暴走防止の安全装置）
- [ ] 設定値がタスクの複雑さに対して **適切** か（短すぎて未完了にならないか、長すぎて暴走しないか）
- [ ] maxTurnsを設定しない場合、その判断が妥当か（信頼性の高い短時間タスク等）
...
### トリガーの検証
- [ ] descriptionのトリガーが実際の用途を網羅し、対象外の依頼と区別できるか（件数は固定しない）
...
### MCP Server設定の妥当性（該当する場合）
- [ ] **mcpServers** で指定されたサーバーの **必要性** が説明できるか
- [ ] インライン定義 vs 名前参照の選択が **適切** か
- [ ] メインコンテキストから分離すべきMCPサーバーが **インライン定義** されているか

### バックグラウンド実行設計（該当する場合）
- [ ] **background** モードの必要性が検討されているか
- [ ] バックグラウンド実行時、パーミッション事前承認の考慮がされているか
- [ ] AskUserQuestion が使えない制約を理解した設計になっているか
