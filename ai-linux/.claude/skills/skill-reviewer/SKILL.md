...
name: skill-reviewer
description: |
  既存スキルの内容妥当性を構造化チェックリストに基づいてレビューし、問題点と改善提案を会話内レポートとして出力するスキル。

  以下の状況で使用:
  (1) ユーザーが「スキルをレビューして」「スキルの検証をして」「スキルの妥当性を確認して」と依頼した時
  (2) ユーザーが明示的に「skill-reviewer スキル」の実行を指示された時
  (3) ユーザーが「このスキルに問題がないか確認して」「スキルの内容をチェックして」と依頼した時
  (4) スキル作成・改善後に「ちゃんとできてるか見て」と依頼された時
  (5) 複数スキルの一括検証を依頼された時
  「レビュー」「検証」「妥当性」「チェック」「問題がないか確認」といったキーワードと
...
既存スキルの内容妥当性を、蓄積したベストプラクティスに基づいて検証するスキル。
...
- 同じスキル集合に含まれる他スキルとの責務・description・トリガー境界の重複
...
`references/checklist.md` を読み込み、チェックリストに沿って検証を実行する。
チェックリストを共通基準として使用する。Phase 3 で確認した現行公式仕様に新しい要件や推奨事項がある場合は、
出典URLと適用理由を明記した「公式仕様の追加項目」として検証へ加える。根拠のない独自項目は追加しない。

検証は以下の6カテゴリで行う（カテゴリ6は Phase 3 で実行）:
1. **構造検証** — frontmatter、ファイル構成、progressive disclosure
2. **記述品質** — 指示の明確さ、why の説明、書き方パターン
3. **description 品質** — 主要用途・トリガー条件・適用境界の明確さ
4. **リソース整合性** — 参照の存在確認、不要ファイルの検出
5. **設計妥当性** — スコープの適切さ、汎用性と具体性のバランス、他スキルとの責務・トリガー境界
...
1. `references/best-practices.md` を読み、記載された出典URLと最終確認日を確認する
2. Codex向けスキルでは、OpenAI公式の Build skills ページを取得し、ローカル資料との差分を確認する
3. 公式ページを取得できない場合はローカル資料で検証を続け、外部確認を実施できなかったことと資料の最終確認日をレポートへ記載する
4. 取得した公式仕様とベストプラクティスを元に、対象スキルの SKILL.md および bundled resources を検証する
5. 公式仕様にチェックリスト未収載の適用可能な項目があれば、出典URLと適用理由を添えて追加検証する
...
- 参照した公式仕様: {URLと確認日 / 外部確認未実施とローカル資料の最終確認日}
...
「ALWAYS/NEVER の多用よりも why を説明する」というアプローチ。
