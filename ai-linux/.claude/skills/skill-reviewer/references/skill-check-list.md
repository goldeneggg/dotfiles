...
**参照:** `subagent-vs-skill.md` の「使い分けのポイント」「補足: クイックチャート」
...
### メタデータの設計
- [ ] name が **kebab-case** で、意図が明確な命名か（agent noun `-er`/`-or`、gerund `-ing`、名詞句いずれも可。`helper`/`utils` 等の曖昧な名前は不可）
- [ ] description に「**何ができるか**」が含まれているか
- [ ] description に「**いつ使うか**」が含まれているか
- [ ] description に **具体的なトリガー例** が含まれているか
- [ ] description が **三人称** で書かれているか（「I can help...」「You can use...」ではなく「Processes...」「Analyzes...」）
- [ ] description がスキル発動を促す **積極的な表現** を含んでいるか（Claudeのundertrigger傾向への対策）
- [ ] description が **100-200字程度** の適切な長さか（短すぎず、長すぎず。最大1024文字）
...
**ワークフロー内の並行実行設計:**
- [ ] **独立した作業ステップが並行実行可能か分析されているか**
  - ✅ PASS: ワークフロー内で独立したステップ（例: 複数ファイルの読み込み、複数観点の調査）がAgent toolによるサブエージェントで並列実行されるよう明記されている
  - ⚠️ WARNING: 並行実行可能なステップがあるが、サブエージェント化が未検討
  - ❌ FAIL: 明らかに並行実行すべき独立ステップが順次実行で記述されている

- [ ] **サブエージェント化の粒度は適切か**
  - ✅ PASS: 各サブエージェントのタスクが明確で、結果の統合方法も記述されている
  - ⚠️ WARNING: サブエージェント化はされているが、タスク粒度が粗い/細かすぎる
  - ❌ FAIL: 依存関係のあるステップまでサブエージェント化している（並列実行不可能なものを並列化）

- [ ] **並行実行の指示が具体的か**
  - ✅ PASS: 「以下のN個のサブエージェントを並列で起動」など、並行実行タイミングと対象が明示されている
  - ⚠️ WARNING: サブエージェント活用の意図はあるが、並行実行の指示が曖昧
  - ❌ FAIL: サブエージェントは使っているが順次実行のみで、並行実行の指示がない
...
**参照:** `subagent-vs-skill.md` の「Skill → Sub Agent」セクション
...
### モデル別テスト
- [ ] 主要モデル（Haiku/Sonnet/Opus）でのテスト実績、またはモデル差異への考慮があるか
  - ✅ PASS: 複数モデルでテスト済み、またはモデル差異を考慮した記述がある
  - ⚠️ WARNING: 単一モデルのみでテスト（他モデルでの動作が未確認）
  - ❌ FAIL: テスト実績なし、かつモデル差異への考慮もない
