...
### 1-03: SKILL.md の情報量と配置
- **基準**: 実行に必要な判断・制約を本文に残し、用途別の詳細を必要に応じて参照できること
- **Pass**: 情報量が用途に見合い、必要な情報を迷わず把握できる
- **Warn**: 重複や用途に無関係な詳細が本文に多く、必要情報の把握を妨げる。該当箇所と影響を示す
- **Fail**: 指示の競合や必要情報の欠落により、実行・完了条件を判断できない。具体的な根拠を示す
- **根拠**: progressive disclosureは必要情報への到達性を高めるために使う。行数は調査の手掛かりとし、行数だけでPass / Warn / Failを決めない
