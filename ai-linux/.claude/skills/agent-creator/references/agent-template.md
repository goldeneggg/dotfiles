...
- [基本テンプレート](#基本テンプレート)
- [読み取り専用エージェントのテンプレート](#読み取り専用エージェントのテンプレート)
- [実行系エージェントのテンプレート](#実行系エージェントのテンプレート)
- [フィールド詳細](#フィールド詳細)
- [設計原則](#設計原則)
...
# 役割、適用する依頼、対象外の境界を記述する
# 例の件数は固定せず、実際の用途を判断できる内容にする
description: |
  [対象]の[具体的な作業]を担当する専門エージェント。
  [適用する依頼・条件]で使用する。[対象外の依頼]は[適切な担当]を使う。
...
- 役割、使用条件、隣接する担当との境界を簡潔に記載する。
- トリガー例は実用途を判断できる範囲で示し、件数を固定しない。
- 自動選択を誘うだけの定型句は加えない。
