...
description: |
  ...
  以下の状況で使用:
    ...
    (3) ユーザーが明示的に「agent-creator スキル」の実行を指示された時
...
- 既存Agentのレビュー・品質評価（→ `agent-reviewer` スキル）
- Skillの作成（→ `skill-creator` スキル）
...
### 次のステップ
`skill-creator` スキルを使用してSkillを作成してください。
```
...
14. **background**: バックグラウンド実行モード。使用時はユーザーへの選択肢提示による確認ができない制約に注意
...
### 要件が不明確な場合
推奨案を含む複数の選択肢を提示してユーザーに確認し、具体化を促す。推測での設計は行わない。
...
`references/subagent-vs-skill.md` のクイックチャートを参照し、該当する項目が多い方を提案。それでも判断できない場合はユーザーに選択を委ねる。
