...
name: task-starter
description: |
  ソフトウェア開発の開始時に、標準ドキュメント構造、仕様書、TODO、依存DAG、クリティカルパス、並行実行計画を生成する。既存TODOの実行はtask-performer、進捗確認はtask-progress-reporterを優先する。新規プロジェクト、機能開発、リファクタリング、バグ修正の計画、タスク分割、ロードマップ作成で使用する。
...
3. **不足情報を段階的に収集**
   - `references/prompt-template.md` の「不足情報の質問順」に従い、重要な項目から1回1〜3問ずつ質問する
   - 選択可能な質問には推奨案と判断材料を添える
   - 任意項目は合理的な案で補い、成果物が大きく変わる場合だけユーザーへ確認する

4. **ドキュメント形式を確定（必須）**
   未指定の場合は、生成ドキュメントを **Markdown** と **HTML** のどちらにするか、選択肢を提示してユーザーに確認する。すでに明示されている場合は再質問しない。
...
2. **specs/ - 仕様書**
   ...
   - **「完了条件 / 成功の定義」セクションは必ず埋める**。ここの内容は Phase 4 で `/goal` コマンドにそのまま渡せる形式で書く必要がある:
     ...
     - 条件は**会話上で実証可能な事実**に落とし込む（例: `<テストコマンド>` の exit 0、Lint エラー 0 件、`PROGRESS.md` の全 `AC-ID` が充足済みなど）
