...
- **会話上で実証可能な事実**で記述する（テスト/Lint コマンドの結果出力、`PROGRESS.md` のAC検証状態など）
...
```text
/goal progresses/ 配下の全 PROGRESS.md で、各TODOに定義されたすべての AC-ID が充足済みであり、`<プロジェクトのテストコマンド>` が exit 0 で成功し、`<lint コマンド>` の出力にエラーが 0 件であることが会話に表示されている
```

OK 例:
- 「`pytest` が `passed` を出力している」
- 「`npm run lint` が `0 problems` を出力している」
- 「`progresses/004-xxx/PROGRESS.md` で全 `AC-ID` が充足済み」
...
- `todos/{ID}/README.*` は静的なタスク定義として更新せず、作業状態と受け入れ条件の検証結果を `progresses/{ID}/PROGRESS.md` に記録する
- 全体の完了は、全タスクの `PROGRESS.md` とテスト・Lint・レビュー結果から判定する

## 完了判定

- すべてのTODOに対応する `PROGRESS.md` の状態が `完了`
- 各TODOに定義されたすべての `AC-ID` が `充足`
- テストとLintがパス
- レビュー承認済み
