...
description: >
  特定のコードベースに対して、新しい技術・ライブラリ・フレームワーク・ツールの導入価値を
  評価するスキル。「Xをプロジェクトに入れるべきか？」「AからBへの移行は妥当か？」
  「Xを採用したら何が変わる？」「自分のリポにXは合う？」といった問いに使う。
  ユーザーが自分のプロジェクト文脈で2つの技術を比較したり、ツールの移行コストや互換性を
  尋ねたり、技術採用の判断を批判的にレビューしてほしい場合にもトリガーする。
  コードベースについて話しながら「Xの導入を考えている」と言うだけでも発動すべき。
  出力は互換性・移行コスト・期待メリット・リスク・「そもそも導入は本当に正当か？」という
  自己批判的分析を含む構造化Markdownレポート。
---
