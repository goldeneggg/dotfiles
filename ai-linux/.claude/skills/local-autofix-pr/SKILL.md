...
description: |
  PRに紐づくCI失敗（lint/型/テスト失敗）と未解決レビューコメント（suggestion・CHANGES_REQUESTED）をローカルで対話的に修正・コミット・pushする日本語スキル。built-in /autofix-pr と異なりpush前のdiff確認・各ステップのAskUserQuestion確認・ブロッカー無し時の早期終了が特徴。

  以下の状況で積極的にトリガーすること:
  - "/local-autofix-pr" "local-autofix-pr 実行して"
...
| 形式 | 例 | 意味 |
|---|---|---|
| 引数なし | `/autofix-pr` | 現ブランチPRをデフォルト挙動で修正 |
| 自由文prompt | `/autofix-pr lintエラーだけ直して` | 修正対象を絞り込む追加指示 |
| `--pr <番号>` | `/autofix-pr --pr 123` | 別のPRを対象にする（要ブランチ切替確認） |
| `--ci-only` | `/autofix-pr --ci-only` | CI失敗のみ修正、レビューコメントは無視 |
| `--reviews-only` | `/autofix-pr --reviews-only` | レビューコメントのみ修正、CIは無視 |
| `--no-push` | `/autofix-pr --no-push` | コミットまでで止め、pushはユーザーに任せる |
...
- 現ブランチと異なる場合、AskUserQuestionで「ブランチを切り替えますか？」を確認
...
整理した指摘事項と修正方針案をユーザーに提示し、AskUserQuestionで以下を確認:
...
- **既存テストがあれば実行**: 修正後に該当テストをローカル実行できる場合は実行
...
を実行し、修正内容をユーザーに見せる。AskUserQuestionで:
...
> 継続的に監視したい場合は `/loop 10m /autofix-pr` を併用可能です。
...
### dirty working tree
未コミット変更がある場合は、AskUserQuestionで「stash / commit / 中断」を確認。
勝手にstashやresetを行わない。
