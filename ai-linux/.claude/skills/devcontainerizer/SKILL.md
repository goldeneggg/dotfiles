...
2. **確認** — モード（実施/計画）と対象範囲を AskUserQuestion で確定する（Phase 1）
...
組み込みの探索ツール（Glob/Grep/Read）と `fd`/`rg` を優先して、以下を収集する。Bash を使う場合はコマンド名に `\` を付ける。
...
## Phase 1: 確認（AskUserQuestion）

偵察結果（2軸の分類・検出した技術スタック・対象アプリ候補）を**先に要約提示**してから、AskUserQuestion で以下を確定する。憶測で進めない。
...
- **確認優先** — 不確実な点は AskUserQuestion で確認してから進む。
