...
description: |
  ...
  (2) ユーザーが明示的に「conversation-analyzer-sqlite スキル」の実行を指示された時
...
- **出力形式**: デフォルトはMarkdown。`scripts/analyze.py --format json` を指定すると、同じレポート本文をJSONの `content` フィールドで返す。CSVが必要な場合はJSON結果を表計算ツール等で変換する

## 実装上の注意

- SQLiteへのアクセスはBashツールで `sqlite3` コマンドを使用する。総合サマリーのように複数観点を横断して集計するような重いクエリ群は、サブエージェントに委譲して実行し、整形済みの集計結果だけをメインに戻させるとよい。単発の確認クエリやスキーマ確認のような軽いクエリは、委譲せずメインで直接実行してよい
- **スキーマ事前確認**: クエリ実行前に `sqlite3 DB ".schema テーブル名"` で実際のカラム名を確認する。本スキルのサンプルSQLとエクスポーター側の実装が乖離している可能性があるため、サンプルSQLのカラム名をそのまま信用しないこと
- **入力値の安全性**: `--project` などのユーザー入力をSQLへ直接連結せず、付属スクリプトのエスケープ済みフィルタを使う。手作業でクエリを組み立てる場合も同じ原則を守ること
