...
以下のルールはすべてのコンテキストファイルに適用する。

### 先頭見出しの形式

ファイルの先頭行は `# CLAUDE.md` のようなファイル名ではなく、`# {repo name}({very short description})` 形式にする。
例: `# my-app(REST API server for e-commerce)`

### 特定AIプロダクト名の排除

"Claude Code"、"Cursor"、"GitHub Copilot" など特定のAIプロダクト名をコンテキストファイル内に含めない。代わりに「AIコーディング支援ツール」「AIエージェント」「LLM」等の汎用的な語句を使用する。これにより、ツール移行時の修正コストを排除し、コンテキストファイルのポータビリティを確保する。
...
- ✅ PASS: 以下の項目が明示的に記載されている
  - プロジェクトの技術スタック（言語、フレームワーク、主要ライブラリ）
  - ディレクトリ構成の概要
  - ビルド・テスト・デプロイの主要コマンド
  - 主要な依存関係やツールチェーンの情報
- ⚠️ WARNING: 上記のうち1-2項目が欠落、または記載が曖昧
- ❌ FAIL: 上記の大半が欠落、またはプロジェクト固有の情報がほぼない
...
- ✅ PASS: 以下の項目が明示的に記載されている
  - コーディング規約またはスタイルガイド（命名規則、フォーマット等）
  - セキュリティや倫理に関する制約（禁止事項、注意事項）
  - AI出力の形式要件（言語、フォーマット、構造）
- ⚠️ WARNING: 上記のうち1項目が欠落、または「一般的なベストプラクティスに従う」等の曖昧な記述のみ
- ❌ FAIL: ガイドラインや制約がまったく定義されていない
...
- ✅ PASS: 以下のうち1つ以上が具体的に記載されている
  - コード例やコマンド例（Good/Badパターン）
  - 期待する出力形式のサンプル
  - プロジェクト固有のワークフロー例
- ⚠️ WARNING: 例が存在するが抽象的、または形式が不統一
- ❌ FAIL: 具体例がまったくない（説明文のみ）
