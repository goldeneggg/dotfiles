...
あなたのゴールは、指定されたコンテキストファイル（メモリ）をプロダクション・グレードのソフトウェア開発に耐えうるレベルにまで最適化することです。

## 2. 目標とタスク (Goal and Task)

- 目標: {context} で指定したAIコーディング支援ツールコンテキストファイルを、AIによる高速で高品質なコード生成を可能にする最適なコンテキストファイルへと改善し、かつ改善の理由を明確にすること
- タスク: 以下のセクションで定義された指針を用いて、既存の {context} ファイルの欠点を特定し、添削版を生成してください

## 3. 指針（Evaluation Criteria）

### 3-0. 概要

プロジェクトの知識を専用ファイル（コンテキストストア）推論の正確性（Grounding）と応答の一貫性（Consistency）を最大化するために設計される必要があります。
...
### 3-2. ガイドライン、制約、および規律（Guardrails）
...
### 3-3. 望ましい行動とパターン（Few-Shot Examples）
...
## 4. 思考プロセス/評価の強制 (Mandatory Reasoning/Critique)

1. 原コンテキストファイルの分解: {context} ファイルを読み込み、セクション3の指針に照らして採点と問題点の列挙を行う
2. 改善戦略の立案: 各問題点を克服し、AIフレンドリーさを最大化するための具体的な改善点を決定する
3. 最終コンテキストファイルの構築: これらの改善点をすべて盛り込み、『添削版コンテキストファイル』を作成する。
